...
%% The Neuron is spawned first, and is then sent the PIds of the Sensor and Actuator that
%% it's connected with. Then the Cortex element is registered and provided with the PIds
%% of all the elements in the NN system.
create() ->
...
%% respectively, it starts waiting for the incoming signals. The neuron expects a vector of
%% length 2 as input, and as soon as the input arrives, the neuron processes the signal and
%% passes the output vector to the outgoing APId.
...
%% The dot function takes a dot product of two vectors, it can operate on a weight vector
%% with and without a bias. When there is no bias in the weight list, both the Input vector
%% and the Weight vector are of the same length. When Bias is present, then when the Input
%% list empties out, the Weights list still has 1 value remaining, its Bias.
...
%% random vector of length 2, which it passes to the connected neuron. In a proper system
%% the sensory signal would not be a random vector but instead would be produced by a function
...
%% also has all the PIds of the elements in the NN system, so that it can terminate the whole system when requested.
