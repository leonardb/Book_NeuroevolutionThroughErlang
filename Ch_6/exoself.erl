...
%% The map function expects for the Cx record to be the leading tuple in the tuple list it
%% reads from the FileName. We create an ets table to map Ids to PIds and back again. Since
%% the Cortex element contains all the Sensor, Actuator, and Neuron Ids, we are able to spawn
%% each neuron using its own gen function, and in the process construct a map from Ids to PIds.
%% We then use link_CerebralUnits to link all non Cortex elements to each other by sending
%% each spawned process the information contained in its record, but with Ids converted to
%% Pids where appropriate. Finally, we provide the Cortex process with all the PIds in the
%% NN system by executing the link_Cortex/2 function. Once the NN is up and running, exoself
%% starts its wait until the NN has finished its job and is ready to backup. When the cortex
%% initiates the backup process it sends exoself the updated Input_PIdPs from its neurons.
%% Exoself uses the update_genotype/3 function to update the old genotype with new weights,
...
%% that belongs to the CerebralUnitType module. We then enter the {Id, PId} tuple into our ETS table for later use.
...
%% 'convert_IdPs2PIdPs' converts the IdPs tuples into tuples that use PIds instead of Ids, such
%% that the Neuron will know which weights are to be associated with which incoming vector signals.
%% The last element is the bias, which is added to the list in a non tuple form. Afterwards, the
%% list is reversed to take its proper order.
...
%% The cortex is initialized to its proper state just as other elements. Because we have not
%% yet implemented a learning algorithm for our NN system, we need to specify when the NN should
...
%% For every {N_Id, PIdPs} tuple the update_genotype/3 function extracts the neuron with
%% the id: N_Id, and updates its weights. The convert_PIdPs2IdPs/3 performs the conversion
%% from PIds to Ids of every {PId, Weights} tuple in the Input_PIdPs list. The updated
%% Genotype is then returned back to the caller.
