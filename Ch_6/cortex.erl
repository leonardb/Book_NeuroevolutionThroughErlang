...
%% state message from the same process that spawned it, exoself. The initial state message
%% contains the sensor, actuator, and neuron PId lists. The message also specifies how many
%% total Sense-Think-Act cycles the Cortex should execute before terminating the NN system.
...
%% The cortex's goal is to synchronize the the NN system such that when the actuators have
%% received all their control signals, the sensors are once again triggered to gather new sensory
%% information. Thus the cortex waits for the sync messages from the actuator PIds in its system,
%% and once it has received all the sync messages, it triggers the sensors and then drops back to
%% waiting for a new set of sync messages. The cortex stores 2 copies of the actuator PIds: the
%% APIds, and the MemoryAPIds (MAPIds). Once all the actuators have sent it the sync messages, it
%% can restore the APIds list from the MAPIds. Finally, there is also the Step variable which
%% decrements every time a full cycle of Sense-Think-Act completes, once this reaches 0, the
%% NN system begins its termination and backup process.
...
%% During backup, cortex contacts all the neurons in its NN and requests for the neuron's Ids
%% and their Input_IdPs. Once the updated Input_IdPs from all the neurons have been accumulated,
